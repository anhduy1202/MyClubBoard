...
export default class PostingService {
    async getPosting(
        ...
        const res = await connection.execute('select posting.*, club.name as club_name, club.logo as club_logo from posting join club on posting.club_id = club.id where posting.id = ?', [id])
        // Not found
        if (res[0].length === 0) {
